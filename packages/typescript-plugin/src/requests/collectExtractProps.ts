import { MpxVirtualCode, isSemanticTokensEnabled } from '@mpxjs/language-core'
...
export function collectExtractProps(
  ...
  const script =
    sourceScript.generated?.languagePlugin.typescript?.getServiceScript(root)
  const maps = script
    ? [...language.maps.forEach(script.code)].map(([_sourceScript, map]) => map)
    : []
  const { sfc } = root
  ...
  sourceFile.forEachChild(function visit(node) {
    if (
      ...
      for (const map of maps) {
        let mapped = false
        for (const source of map.toSourceLocation(
          name.getEnd() - (isTsPlugin ? sourceScript.snapshot.getLength() : 0),
        )) {
          if (

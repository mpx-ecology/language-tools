...
export interface ScriptSetupRanges
  extends ReturnType<typeof parseScriptSetupRanges> {}
...
export function parseScriptSetupRanges(
  ...
  const leadingCommentRanges =
    ts.getLeadingCommentRanges(text, 0)?.reverse() ?? []
  const leadingCommentEndOffset =
    leadingCommentRanges.find(range =>
      tsCheckReg.test(text.slice(range.pos, range.end)),
    )?.end ?? 0
  ...
  function visitNode(node: ts.Node, parents: ts.Node[]) {
    ...
    if (ts.isCallExpression(node) && ts.isIdentifier(node.expression)) {
      ...
      const isDefineModel =
        mpxCompilerOptions.macros.defineModel.includes(callText)
      if (isDefineModel || callText === 'defineProp') {
      ...
      } else if (mpxCompilerOptions.macros.defineProps.includes(callText)) {
        defineProps = {
        ...
        if (
          ...
          mpxCompilerOptions.macros.withDefaults.includes(
            _getNodeText(parent.expression),
          )
        ) {
      ...
      } else if (mpxCompilerOptions.macros.withDefaults.includes(callText)) {
      ...
      } else if (mpxCompilerOptions.macros.defineExpose.includes(callText)) {
  ...
  function parseCallExpression(node: ts.CallExpression) {
    return {
      ...
      typeArg: node.typeArguments?.length
        ? _getStartEnd(node.typeArguments[0])
        : undefined,
    }
  }

  function parseCallExpressionAssignment(
...
function getClosestMultiLineCommentRange(
  ...
  const comment = ts
    .getLeadingCommentRanges(ast.text, node.pos)
    ?.reverse()
    .find(
      range =>
        range.kind === (3 satisfies ts.SyntaxKind.MultiLineCommentTrivia),
    )

  ...
  ts.forEachChild(ast, raw => {
    if (ts.isExportAssignment(raw)) {
      let node:
        | ts.AsExpression
        | ts.ExportAssignment
        | ts.ParenthesizedExpression = raw
      while (
        isAsExpression(node.expression) ||
      ...
      if (obj) {
        ...
        exportDefault = {
          ...
          componentsOption: componentsOptionNode
            ? _getStartEnd(componentsOptionNode)
            : undefined,
          componentsOptionNode: withNode ? componentsOptionNode : undefined,
          directivesOption: directivesOptionNode
            ? _getStartEnd(directivesOptionNode)
            : undefined,

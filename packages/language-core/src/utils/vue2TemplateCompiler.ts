...
export const compile: typeof CompilerDOM.compile = (template, options = {}) => {
  ...
  const vue2Result = Vue2TemplateCompiler.compile(template, {
...
function baseCompile(
  ...
  if (!prefixIdentifiers && options.cacheHandlers) {
    onError(
      CompilerDOM.createCompilerError(
        49 satisfies CompilerDOM.ErrorCodes.X_CACHE_HANDLER_NOT_SUPPORTED,
      ),
    )
  }
  if (options.scopeId && !isModuleMode) {
    onError(
      CompilerDOM.createCompilerError(
        50 satisfies CompilerDOM.ErrorCodes.X_SCOPE_ID_NOT_SUPPORTED,
      ),
    )
  }
  ...
  const [nodeTransforms, directiveTransforms] =
    CompilerDOM.getBaseTransformPreset(prefixIdentifiers)

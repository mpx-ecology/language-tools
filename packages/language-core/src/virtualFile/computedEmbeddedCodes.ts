...
import { MpxEmbeddedCode } from './embeddedFile'
...
export function computedEmbeddedCodes(
  ...
  const getStructuredResult = computed(() => {
    ...
    for (const { code } of remain) {
      console.error(
        'Unable to resolve embedded: ' + code.parentCodeId + ' -> ' + code.id,
      )
    }
    ...
    function findParentStructure(
      ...
      for (const child of current) {
        ...
        const parent = findParentStructure(id, child.embeddedCodes ?? [])
        if (parent) {
          return parent
        }
      }
...
function computedPluginEmbeddedCodes(
  ...
  const computeds = new Map<
    string,
    () => { code: MpxEmbeddedCode; snapshot: ts.IScriptSnapshot }
  >()
  const getComputedKey = (code: { id: string; lang: string }) =>
      ...
      for (const codeInfo of embeddedCodeInfos) {
        if (!computeds.has(getComputedKey(codeInfo))) {
          computeds.set(
            ...
            computed(() => {
              const content: Code[] = []
              const code = new MpxEmbeddedCode(
                codeInfo.id,
                codeInfo.lang,
                content,
              )
              ...
              const snapshot: ts.IScriptSnapshot = {
                ...
                getChangeRange(oldSnapshot) {
                  if (!changeRanges.has(oldSnapshot)) {
                    ...
                    const oldText = oldSnapshot.getText(
                      0,
                      oldSnapshot.getLength(),
                    )
                    const changeRange = fullDiffTextChangeRange(
                      oldText,
                      newText,
                    )
                    if (changeRange) {
      ...
      for (let i = 0; i < mappings.length; i++) {
        ...
        if (mapping.data.__combineOffset !== undefined) {
          ...
          if (typeof offsetMapping === 'string' || !offsetMapping) {
            throw new Error(
              'Invalid offset mapping, mappings: ' +
                mappings.length +
                ', i: ' +
                i +
                ', offset: ' +
...
function fullDiffTextChangeRange(
  oldText: string,
  newText: string,
): ts.TextChangeRange | undefined {
  for (
    let start = 0;
    start < oldText.length && start < newText.length;
    start++
  ) {
    if (oldText[start] !== newText[start]) {
      ...
      for (
        let i = 0;
        i < oldText.length - start && i < newText.length - start;
        i++
      ) {
        if (
          oldText[oldText.length - i - 1] !== newText[newText.length - i - 1]
        ) {
          break
        }
        end--

...
export function* generateVSlot(
  ...
  if (slotDir || node.children.length) {
    ...
    if (slotDir) {
      if (
        slotDir.arg?.type === CompilerDOM.NodeTypes.SIMPLE_EXPRESSION &&
        slotDir.arg.content
      ) {
      ...
      yield* wrapWith(start, end, ctx.codeFeatures.navigation, `default`)
    }
    yield `: ${slotVar} } = ${ctx.currentComponent.ctxVar}.slots!${endOfLine}`
  }

  if (slotDir?.exp?.type === CompilerDOM.NodeTypes.SIMPLE_EXPRESSION) {
    const slotAst = createTsAst(
      options.ts,
      slotDir,
      `(${slotDir.exp.content}) => {}`,
    )
  ...
  if (options.mpxCompilerOptions.strictSlotChildren && node.children.length) {
    const { start, end } = getElementInnerLoc(options, node)
...
function* generateSlotParameters(
  ...
  for (const { name, type } of expression.parameters) {
    if (type) {
      modifies.push([[``], name.end, type.end])
